      ...
      try {
        ...
        const combined: Partial<Device> = {
          ...
          macAddress: serverData?.server?.serverMacs
            ? (Object.values(serverData.server.serverMacs)[0] as string)
            : undefined,
          ...
          operatingSystem: serverData?.parsedUA?.os?.name ?? undefined,
          osVersion: serverData?.parsedUA?.os?.version ?? undefined,
          browser: serverData?.parsedUA?.client?.name ?? undefined,
          browserVersion: serverData?.parsedUA?.client?.version ?? undefined,
        ...
        if (mounted) setError(e?.message || "failed to collect device info");
      } finally {
